...
class DynamicsRecovery(BaseDynamics):
    ...
    def initialize_WX(self, method):
        ...
        if method == 'rand':
        ...
        elif method == 'knockTF_Log2FC':
            weight = self.knockTF_Log2FC / np.dot(self.x_f, self.knockTF_Log2FC.reshape(-1,1)).std()
            ...
            tmp1 = self.max_yf_x.copy()
            tmp2 = self.gamma * self.max_yf
            lb = -self.WX_thres
            hb = self.WX_thres
            weight = lsq_linear(tmp1, tmp2, bounds=(lb, hb), method='bvls', lsmr_tol='auto', verbose=0)
            weight = weight.x
    ...
    def init_par(self, alpha=None, beta=None, omega=None, theta=None, gamma=None):
        ...
        if not self.init_weight_method is None:
            self.WX, self.weights_init = self.initialize_WX(method=self.init_weight_method) 
        ...
        #if self.gamma < self.gamma_thres:
        #    self.gamma = self.gamma_thres
        self.t = self.assign_time(self.WX, self.y, self.alpha, self.beta, self.omega, self.theta, 
            self.gamma, self.filtered, norm_std=True)
        ...
        self.loss = [self.get_loss(refit_time=False, norm_std=True, reg=self.reg)]
        return
    ...
    def fit(self):
        if self.fix_theta:
            self.search_and_fit_no_theta(reg=self.reg)
        else:
            self.search_and_fit(reg=self.reg)
        self.WX = use_model(self.WX_model, self.x, method=self.WX_method, bias=self.delta)
        self.t = self.assign_time(self.WX, self.y, self.alpha, self.beta, self.omega, self.theta,
            self.gamma, self.filtered, norm_std=True)
        ...
        self.weights = np.ravel(self.WX_model)
        self.weights_final = self.weights * self.x_f.sum(0) 
        self.weights_final = self.weights_final / abs(self.weights_final).max()
        #if :
        #    self.WX_model = - self.WX_model
        #    self.fit_paras_(reg=self.reg)
    ...
    def fit_paras_(self, **kwargs):
        ...
        bounds=((0, None), (None, None), (-np.pi, np.pi), (self.gamma_thres, None))
    ...
    def search_and_fit_no_theta(self, **kwargs):
        omega = 2*np.pi 
        theta = 0
        self.save_par()
        self.best_loss = MAX_LOSS
        self.init_par(omega=omega, theta=theta)
        self.fit_paras_no_theta_(**kwargs)
        if self.loss[-1] < self.best_loss:
            self.save_par()
        self.load_par()
        self.tmp_best_loss = self.best_loss
    ...
    def fit_paras_no_theta_(self, **kwargs):
        def mse(data):
            return self.get_mse(alpha=data[0], beta=data[1], gamma=data[2], **kwargs)
        data_0, cb = np.array([self.alpha, self.beta, self.gamma]), self.cb_fit_paras_no_theta_
        bounds=((0, None), (None, None), (self.gamma_thres, None))
        res = minimize(mse, data_0, callback=cb, bounds=bounds, **self.simplex_kwargs)
        self.cb_fit_paras_no_theta_(res.x)
    def cb_fit_paras_no_theta_(self, data):
        self.update(alpha=data[0], beta=data[1], gamma=data[2], refit_time=True, update_weight=True)
    ...
    def update(
        ...
        loss = self.get_loss(t, alpha, beta, omega, theta, gamma, delta, WX_model, refit_time=False, norm_std=True, reg=self.reg)
    ...
    def assign_time(
        ...
        num = np.clip(len(y), self.n_time_points, self.n_time_points*2)
...
default_pars_names = ["alpha", "beta", "omega", "theta", "gamma", "delta", "likelihood", "varx", "scaling_y"]
default_pars_names_vec = ["scaling", "weights", "weights_init", "weights_final"]
...
def recover_dynamics(
    ...
    **kwargs,
    ...
    alpha, beta, omega, theta, gamma, delta, likelihood, varx, scaling_y = pars[:-4]
    scaling, weights, weights_init, weights_final = pars[-4], pars[-3], pars[-2], pars[-1]
    ...
    for ix, dm in zip(idx, dms): 
        ...
        if WX_method in ['lsq_linear']:
            ...
            weights_final[ix, :len(weight)] = np.ravel(dm.weights_final)
    ...
    _pars = [
        alpha, beta, omega, theta, gamma, delta, likelihood, varx, scaling_y, \
        scaling, weights, weights_init, weights_final
    ]
...
def latent_time(
    ...
    if t_key not in adata.layers.keys():
