...
def get_metric_pseudotime(adata, t_key="latent_t"):
    ...
    for i in range(n_genes):
        correlation, _ = scipy.stats.spearmanr(
            adata.layers[t_key][:, i], adata.obs["velocity_pseudotime"]
        )
...
def main(args):
    ...
    expanded_scaling_y = np.expand_dims(np.array(adata.var["fit_scaling_y"]), 0).repeat(
        n_cells, axis=0
    )
    ...
    print(adata_copy_1)
    adata_copy_1.write(os.path.join(args.result_path, "rc.h5ad"))
    ...
    adata_copy = adata_copy[
        :, adata_copy.var["spearmanr_pseudotime"] > args.spearmanr_thres
    ]
    ...
    print(adata_copy)
    adata_copy.write(os.path.join(args.result_path, "TFvelo.h5ad"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--loss_percent_thres", type=int, default=50, help="max loss of each gene"
    )
    parser.add_argument(
        "--spearmanr_thres", type=float, default=0.8, help="min spearmanr"
    )
    parser.add_argument(
